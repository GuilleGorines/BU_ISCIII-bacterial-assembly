...
def parse_args(args=None):
    ...
    Epilog = (
        """Example usage: python sra_runinfo_to_samplesheet.py <FILE_IN> <FILE_OUT>"""
    )

    parser = argparse.ArgumentParser(description=Description, epilog=Epilog)
    parser.add_argument(
        "FILE_IN",
        help="Input metadata file created from 'fetch_sra_runinfo.py' script.",
    )
        ...
        if line:
            ...
            if library == "SINGLE":
                ...
                if fastq_files:
                    ...
                    if (
                        fq_files[0].find("_1.fastq.gz") != -1
                        and fq_files[1].find("_2.fastq.gz") != -1
                    ):
                        sample_info = (
                            ["0", "1", "1"] + fq_files + fastq_md5.split(";")[-2:]
                        )
                    else:
                        print(
                            "Invalid FastQ files found for database id:'{}'!.".format(
                                run_id
                            )
                        )
                else:
            ...
            if sample_info:
                if db_id not in sampleRunDict:
                    ...
                    if sample_info in sampleRunDict[db_id]:
                        print(
                            "Input run info file contains duplicate rows!\nLine: '{}'".format(
                                line
                            )
                        )
                    else:
