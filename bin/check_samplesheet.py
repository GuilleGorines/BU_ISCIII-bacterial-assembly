...
def parse_args(args=None):
    Description = \
        "Reformat nf-core/viralrecon samplesheet file and check its contents."
    Epilog = \
        """Example usage: python check_samplesheet.py <FILE_IN> <FILE_OUT>"""
...
def print_error(error, line):
    print(
        "ERROR: Please check samplesheet -> \
...
def check_samplesheet(FileIn, FileOut):
        ...
        if line:
            ...
            if len(lspl) != len(header):
                print_error(
                    ...
                    (minimum = {})!".format(len(header)),
                    line,
                )

            numCols = len([x for x in lspl if x])
            if numCols < 2:
                print_error("Invalid number of populated columns \
                    (minimum = 2)!", line)
            ...
            for fastq in fastQFiles:
                if fastq:
                    ...
                    if fastq[-9:] != ".fastq.gz" and fastq[-6:] != ".fq.gz":
                        print_error(
                            "FastQ file does not have extension \
                            '.fastq.gz' or '.fq.gz'!",
                            line,
                        )
            ...
            sample_info = (
                []
            )  # [single_end, is_sra, is_ftp, fastq_1, fastq_2, md5_1, md5_2]
    ...
    if len(sampleRunDict) > 0:
        ...
        for sample in sorted(sampleRunDict.keys()):
            ...
            # Check that multiple runs of the same sample
            # are of the same datatype
            if not all(
                x[:2] == sampleRunDict[sample][0][:2]
                for x in sampleRunDict[sample]
            ):
                print_error(
            ...
            for idx, val in enumerate(sampleRunDict[sample]):
                text = ",".join(["{}_T{}".format(sample, idx + 1)] + val)
                + "\n"
                fout.write(text)
        fout.close()
